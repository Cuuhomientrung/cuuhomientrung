...
def is_vietnamese(name):
        accents = ['à', 'á', 'ạ', 'ả', 'ã', 'ó', 'ò', 'ỏ', 'ọ', 'õ', 'ô', 'ồ', 'ố', 'ổ', 'ộ', 'ơ', 'ờ', 'ớ', 'ở', 'ợ', 'ê', 'ề', 'ế', 'ể', 'ệ', 'ì', 'í', 'ỉ', 'ị', 'ù', 'ú', 'ụ', 'ủ', 'ư', 'ừ', 'ú', 'ủ', 'ự', 'ữ', 'ỳ', 'ý', 'ỷ', 'ỹ', 'ỵ']
        is_vietnamese = False
        for ch in accents:
            if ch in name:
                is_vietnamese = True
        ...
        for tinh in Tinh.objects.all():
            if not is_vietnamese(tinh.name):
                if has_special_character(tinh.name):
                    ...
                    choice = input(f"Do you want to delete {tinh.name} ? (Y/N)")
        ...
        for huyen in Huyen.objects.all():
            if not is_vietnamese(huyen.name):
                if has_special_character(huyen.name):
                    ...
                    choice = input(f"Do you want to delete {huyen.name} ? (Y/N)")
                    if 'y' in choice.lower():
                        huyen.delete()
